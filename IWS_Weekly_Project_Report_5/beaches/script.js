...
function displayTrailDetails(place) {
    const reviews = place.reviews ? place.reviews.map(review => `<strong>${review.author_name}</strong>: ${review.text}`).join('<br><br>'): 'No reviews available';
    const detailsHTML = `
        <div class="popup" id="beachPopup">
    ...
    document.getElementById('beachList').classList.add('blur-element');
    document.querySelector('button').classList.add('blur-element');
    document.getElementById('popupContainer').innerHTML = detailsHTML;
    document.getElementById('beachPopup').style.display = 'block';
}

function closePopup() {
    document.getElementById('beachPopup').style.display = 'none';
    ...
    document.getElementById('beachList').classList.remove('blur-element');
...
function callback(results, status) {
    if (status === google.maps.places.PlacesServiceStatus.OK) {
        const listElement = document.getElementById('beachList');
